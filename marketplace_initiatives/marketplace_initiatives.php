...
<body>
                        ...
                        <aside class="sidebar shop-sidebar sticky-sidebar-wrapper sidebar-fixed">
                            ...
                            <div class="sidebar-content scrollable">
                                ...
                                <div class="sticky-sidebar">
                                    ...
                                    <?php
                                    $sql = "SELECT cat_id FROM marketplace_initiatives WHERE site_id = $this_site_id AND status = 1 AND verified = 1";
                                    ...
                                    <div class="widget widget-collapsible">
                                        ...
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            $count++;
                                        ?>
                                            <div class="form-check" style="font-size: 14px; padding: 0.5em;">
                                                <input class="form-check-input" type="checkbox" value="" id="flexCheckDefaultCat<?php echo $count; ?>" id="category<?php echo $count ?>" check-status="unchecked" onchange="fetchInitiatives(event, '<?php echo $row['cat_id']; ?>', 1, 'flexCheckDefaultCat<?php echo $count; ?>')">
                                                <label class="form-check-label" for="flexCheckDefaultCat<?php echo $count; ?>">
                                                    <?php echo $row['cat_id']; ?>
                                                </label>
                                            </div>
                                        <?php } ?>
                                    ...
                                    <?php
                                    $sql = "SELECT type FROM marketplace_initiatives WHERE site_id = $this_site_id AND status = 1 AND verified = 1";
                                    $res = mysqli_query($conn, $sql);
                                    ?>
                                    <div class="widget widget-collapsible">
                                        ...
                                        ?>
                                            <div class="form-check" style="font-size: 14px; padding: 0.5em;">
                                                <input class="form-check-input" type="checkbox" value="" id="flexCheckDefaultType<?php echo $count; ?>" id="type<?php echo $count ?>" check-status="unchecked" onchange="fetchInitiatives(event, '<?php echo $row['type']; ?>', 2, 'flexCheckDefaultType<?php echo $count; ?>')">
                                                <label class="form-check-label" for="flexCheckDefaultType<?php echo $count; ?>">
                                                    <?php echo $row['type']; ?>
                                                </label>
                                            </div>
                                        <?php } ?>
                                    ...
                                    <div class="widget widget-collapsible" style="margin-top: 1em;">
                                        <div class="toolbox-item toolbox-sort select-box text-dark">
                                            <label>Created date : &nbsp;</label>
                                            <select name="orderby" class="form-control" id="selected-date-order" onchange="fetchInitiatives(event, this.value, 3, '')">
    ...
    <script>
        ...
        var tot;
        var orderBy = 'default'
        // fetchInitiatives(event, this.value, 3, '')
        function fetchInitiatives(event, category, type, id) {
            if (type === 3) {
                orderBy = category;
                if(filters.size == 0){
                    reloadDefault();
                ...
                var checkClass = $('#' + id).attr('check-status');

                if (checkClass == 'unchecked') {
                    filters.set(category, type);
                    $('#' + id).attr('check-status', 'checked')
                } else if (checkClass == 'checked') {
                    filters.delete(category);
                    $('#' + id).attr('check-status', 'unchecked')
                }
            }

            if (filters.size == 0) {
                $("#product-holder-section").html(initial);
                $("#current-showing").html(tot);
                $("#total-showing").html(tot);
                first = 1;
                return;
            }

            $.ajax({
                url: "fetch_initiatives.php",
                type: "POST",
                data: {
