...
<body>
                    ...
                    <div class="shop-content row gutter-lg mb-10">
                                ...
                                <div class="sticky-sidebar">
                                    ...
                                    <div class="widget widget-collapsible">
                                        ...
                                        <ul class="widget-body filter-items search-ul">
                                            <?php
                                            $sql = "SELECT cat_id FROM affiliate_products WHERE site_id = $this_site_id AND status = 0 AND verified = 0";
                                            $res = mysqli_query($conn, $sql);

                                            while ($row = mysqli_fetch_assoc($res)) {
                                            ?>
                                                <li><a href="#" onclick="fetchResult(event, '<?php echo $row['cat_id']; ?>', 1)"><?php echo $row['cat_id']; ?></a></li>
                                            <?php } ?>
                                        </ul>
                                    </div>

                                    <div class="widget widget-collapsible">
                                        <h3 class="widget-title"><span>Source</span></h3>
                                        <ul class="widget-body filter-items search-ul">
                                            <?php
                                            $sql = "SELECT source FROM affiliate_products WHERE site_id = $this_site_id AND status = 0 AND verified = 0";
                                            $res = mysqli_query($conn, $sql);

                                            while ($row = mysqli_fetch_assoc($res)) {
                                            ?>
                                                <li><a href="#" onclick="fetchResult(event, '<?php echo $row['source']; ?>', 2)"><?php echo $row['source']; ?></a></li>
                                            <?php } ?>
                                        </ul>
                                    </div>
                        ...
                        <div class="main-content">
                            ...
                            <div class="product-wrapper row cols-lg-4 cols-md-3 cols-sm-2 cols-2" id="product-holder-section">
                                <?php
                                $sql = "SELECT * FROM affiliate_products WHERE site_id = $this_site_id AND status = 0 AND verified = 0";
                                $res = mysqli_query($conn, $sql);
                                $countCurrent = 0;

                                while ($row = mysqli_fetch_assoc($res)) {
                                ?>
                                    <div class="product-wrap" style="margin: 0px 25px; width: 1000px; margin-left: 0;">
                                        <div class="product text-center">
                                            <figure class="product-media">
                                                <a href="product-default.html">
                                                    <?php echo $row['product_url']; ?>
                                                </a>
                                            ...
                                            <div class="product-details">
                                                ...
                                                <h3 class="product-name">
                                                    <a href="product-default.html"><?php echo $row['product_title']; ?></a>
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function lazyLoadingCategory(ev, cat, id, st) {
            ev.preventDefault();
            $.ajax({
                url: "fetch_affiliate_product.php",
                type: "POST",
                data: {
                    category: cat,
                    id: id,
        ...
        function fetchResult(ev, cat, id) {
            currentShowing = 0;
            first = 1;
            var load_flag = 0;
            lazyLoadingCategory(ev, cat, id, load_flag);
            load_flag += 4;
            $(window).scroll(function() {
                var windowHeight = $(window).height();
                var windowTop = $(window).scrollTop() + 1;
                if (windowTop >= windowHeight) {
                    windowHeight = $(window).height() + windowTop - 1;
                    lazyLoadingCategory(ev, cat, id, load_flag);
                    load_flag += 4;
                }
            });
