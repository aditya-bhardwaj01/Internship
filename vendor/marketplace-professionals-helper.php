...
if (isset($_POST["action"])) {
	$sql = "SELECT a.*, c.name, c.profile_pic from marketplace_professionals a INNER JOIN vendor_type b INNER JOIN vendor c ON a.site_id = $this_site_id AND a.professional_type = b.id AND b.parent_type = 2 AND a.vendor_id = c.id ";
					...
					<div class="product-wrap" id="<?php echo $res["id"]; ?>">
						<div class="product product-simple text-center" id="<?php echo $res["id"]; ?>">
							<figure class="product-media">
								<a href="profile?id=<?php echo urlencode(base64_encode($res['vendor_id'])) ?>">
									<img src="<?php echo $res["profile_pic"]; ?>" alt="Consultant Profile Photo" width="295" height="335" />
									<input type="checkbox" name="" class="profileSelector" value="<?php echo $res["id"]; ?>" id="" style="position: absolute; top: 0; right: 0; height: 20px; width: 20px;">
								</a>
								<?php
								...
								?>
									<div class="product-label-group">
										<label class="product-label label-discount"><?php echo (htmlspecialchars($res["label"])); ?></label>
									</div>
								<?php } ?>
							...
							<div class="product-details">
								<h4 class="product-name"><a href="profile.php"><?php echo (htmlspecialchars($res["name"])); ?></a></h4>
							</div>
...
else if (isset($_POST["quick"])) {
	$id = realEscape((($_POST["id"])));
	$sql = "SELECT a.*, c.name, c.profile_pic from marketplace_professionals a INNER JOIN vendor_type b INNER JOIN vendor c ON a.site_id = $this_site_id AND a.professional_type = b.id AND b.parent_type = 2 AND a.vendor_id = c.id AND a.id = '$id' ";
				...
				<div class="row gutter-lg" id="view">
						...
						<div class="product-gallery product-gallery-sticky mb-0">
							<div class="product-single-swiper swiper-container swiper-theme nav-inner row cols-1 gutter-no">
								<figure class="product-image">
									<img src="<?php echo $res["profile_pic"]; ?>" data-zoom-image="<?php echo $res["profile_pic"]; ?>" alt="Consultant Profile Photo" width="800" height="900">
								</figure>
							</div>
						</div>
					</div>
					<div class="col-md-9 overflow-hidden p-relative">
						<div class="product-details scrollable pl-0">
							...
							<h6 class="product-name"><a href="profile.php"><?php echo $conn->real_escape_string(htmlspecialchars($res["bio"])); ?></a></h6>
